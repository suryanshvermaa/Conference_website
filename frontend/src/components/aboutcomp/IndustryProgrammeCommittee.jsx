...
export const IndustryProgrammeCommitteeLoader = () => {
  const cards = [1, 2, 3, 4];
  return (
...
const IndustryProgrammeCommittee = () => {
  ...
  useEffect(() => {
    async function fetchAllCommitteeMembers() {
      ...
      try {
        ...
        setCookie("ipcMembersCache", res.data.members, {
          path: "/",
          maxAge: 86400,
        }); // Cache for 1 day
  ...
  if (loading) {
    return <IndustryProgrammeCommitteeLoader />;
  } else {
    return (
      <div className="min-h-screen bg-gray-50 flex justify-center items-center py-12 px-4 sm:px-6 lg:px-8">
        <section className="w-full max-w-7xl bg-white rounded-2xl shadow-2xl p-8 md:p-12">
...
export default IndustryProgrammeCommittee;
